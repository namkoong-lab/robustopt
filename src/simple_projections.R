...
project_onto_chi_square_ball <- function(w, rho, tol = 1e-10) {
  stopifnot(rho > 0)
  rho = as.double(rho)
  
  # sort in decreasing order
  w_sort = w[order(w, decreasing=T)]
  
  w_sort_cumsum = cumsum(w_sort)
  w_sort_sqr_cumsum = cumsum(w_sort ^ 2)
  nn = as.double(length(w_sort))

  lam_min = 0.0
  lam_max = (1/nn) * (nn * w_sort[1] / sqrt(2. * rho + 1.) - 1.)
  lam_init_max = lam_max

  if (lam_max <= 0) { # optimal lambda is 0
    inner_solve = solve_inner_eta(w_sort, w_sort_cumsum, nn, 0., rho)
    eta = inner_solve$eta
    ind = inner_solve$ind
    p = w - eta
    low_inds = p < 0
  ...
  while ((lam_max - lam_min) > tol * lam_init_max) {
    ...
    inner_solve = solve_inner_eta(w_sort, w_sort_cumsum, nn, lam, rho)
    eta = inner_solve$eta
    ind = inner_solve$ind

    # compute norm(p(lam))^2 * (1+lam * nn)^2
    thresh = .5 * nn * (w_sort_sqr_cumsum[ind+1] - 2. * eta * w_sort_cumsum[ind+1] + (eta ^ 2) * (ind+1.))
...
solve_inner_eta <- function(w_sort, w_sort_cumsum, nn, lam, rho) {
  fs = w_sort - (w_sort_cumsum - (1. + lam * nn)) / as.vector(1:nn)
  ind = sum(fs > 0)-1
  return(list(eta=(1 / (ind+1.)) * (w_sort_cumsum[ind+1] - (1. + lam * nn)), ind=ind))
}

test_projections <- function(tol = 1e-5) {
  ...
  kkt_error = abs(rho + .5 - .5 * nn * vec_norm(p1) ^ 2) + abs(sum(p1) - 1.0) + abs(sum(p1[p1 < 0]))
  
  if (kkt_error > tol) {
    print(paste("Test failed for nn=", nn, " and rho=", rho, " with error=", vec_norm(p1_desired - p1), " and kkt_error = ", kkt_error, sep=""))
  } else {
  ...
  kkt_error = abs(rho + .5 - .5 * nn * vec_norm(p2)**2) + abs(sum(p2) - 1.0) + abs(sum(p2[p2 < 0]))
  
  if(kkt_error > tol) {
    print(paste("Test failed for nn=", nn, " and rho=", rho, " with error=", vec_norm(p2_desired - p2), " and kkt_error = ", kkt_error, sep=""))
  } else {
    num_passed = num_passed + 1
  }

  print(paste("Passed ", num_passed, " out of 2 tests", sep=""))
}
